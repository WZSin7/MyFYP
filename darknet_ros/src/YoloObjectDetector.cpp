  ...
  if (cam_image) {
    {
      ...
      camImageCopy_ = cam_image->image.clone(); //Clone the mat
    }
    {
...
void YoloObjectDetector::checkForObjectsActionGoalCB() {
  ...
  try {
    cam_image = cv_bridge::toCvCopy(imageAction); //Convert a sensor_msgs::Image message to an OpenCV-compatible CvImage, copying the image data.
  } catch (cv_bridge::Exception& e) {
    ROS_ERROR("cv_bridge exception: %s", e.what());
    return;
  }
...
IplImageWithHeader_ YoloObjectDetector::getIplImageWithHeader() {
  //IplImage* ROS_img = new IplImage(camImageCopy_); //Error , (camImageCopy_ from line 175, 205)
